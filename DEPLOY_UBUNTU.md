# Hướng dẫn Deploy Real-time Brute-Force Detection trên Ubuntu SIEM Wazuh

## Thông tin Deployment

- **Script Path**: `/root/ai-brute/rebornAI/rebornAI/realtime_bruteforce_detector.py`
- **Project Directory**: `/root/ai-brute/rebornAI/rebornAI/`
- **Input Log**: `/opt/ai-bruteforce/brute.log`
- **Output Log**: `/var/ossec/logs/brute.log`
- **Service Name**: `bruteforce-detector`

## Yêu cầu

1. **Hệ thống**: Ubuntu (tested on Ubuntu 20.04/22.04)
2. **Python**: Python 3.8+
3. **Dependencies**: pandas, scikit-learn, joblib, numpy
4. **Model đã train**: File `models/optimized_bruteforce_detector.pkl` phải tồn tại
5. **Quyền root**: Cần để tạo service và thư mục system

## Bước 1: Chuẩn bị Project

### Upload code lên server

```bash
# Tạo thư mục project
sudo mkdir -p /root/ai-brute/rebornAI/rebornAI
cd /root/ai-brute/rebornAI/rebornAI

# Upload các file cần thiết:
# - realtime_bruteforce_detector.py
# - optimized_bruteforce_detector.py
# - bruteforce_tracker.py
# - models/optimized_bruteforce_detector.pkl
# - models/optimized_bruteforce_metadata.json
```

### Cấu trúc thư mục

```
/root/ai-brute/rebornAI/rebornAI/
├── realtime_bruteforce_detector.py
├── optimized_bruteforce_detector.py
├── bruteforce_tracker.py
├── check_model.py
├── models/
│   ├── optimized_bruteforce_detector.pkl
│   └── optimized_bruteforce_metadata.json
└── deploy_realtime.sh
```

## Bước 2: Cài đặt Dependencies

```bash
# Cài đặt Python dependencies
sudo pip3 install pandas scikit-learn joblib numpy

# Hoặc sử dụng virtual environment (khuyến nghị)
cd /root/ai-brute/rebornAI/rebornAI
python3 -m venv venv
source venv/bin/activate
pip install pandas scikit-learn joblib numpy
```

**Lưu ý**: Nếu dùng virtual environment, cần update service file để dùng Python từ venv.

## Bước 3: Train Model (nếu chưa có)

```bash
cd /root/ai-brute/rebornAI/rebornAI

# Upload file normal.log (training data)
# Sau đó train model:
python3 optimized_bruteforce_detector.py
```

Model sẽ được lưu tại: `models/optimized_bruteforce_detector.pkl`

## Bước 4: Deploy Service

### Cách 1: Sử dụng script tự động (khuyến nghị)

```bash
cd /root/ai-brute/rebornAI/rebornAI
chmod +x deploy_realtime.sh
sudo ./deploy_realtime.sh
```

### Cách 2: Deploy thủ công

#### 1. Tạo thư mục cần thiết:

```bash
sudo mkdir -p /opt/ai-bruteforce
sudo mkdir -p /var/ossec/logs
sudo chmod 755 /opt/ai-bruteforce
sudo chmod 755 /var/ossec/logs
```

#### 2. Tạo file log input (nếu chưa có):

```bash
sudo touch /opt/ai-bruteforce/brute.log
sudo chmod 644 /opt/ai-bruteforce/brute.log
```

#### 3. Tạo systemd service file:

```bash
sudo nano /etc/systemd/system/bruteforce-detector.service
```

Thêm nội dung:

```ini
[Unit]
Description=Real-time Brute-Force Detection Service
After=network.target

[Service]
Type=simple
User=root
WorkingDirectory=/root/ai-brute/rebornAI/rebornAI
ExecStart=/usr/bin/python3 /root/ai-brute/rebornAI/rebornAI/realtime_bruteforce_detector.py
Restart=always
RestartSec=10
StandardOutput=journal
StandardError=journal

[Install]
WantedBy=multi-user.target
```

**Nếu dùng virtual environment**, thay ExecStart:

```ini
ExecStart=/root/ai-brute/rebornAI/rebornAI/venv/bin/python3 /root/ai-brute/rebornAI/rebornAI/realtime_bruteforce_detector.py
```

#### 4. Khởi động service:

```bash
sudo systemctl daemon-reload
sudo systemctl enable bruteforce-detector
sudo systemctl start bruteforce-detector
```

## Bước 5: Kiểm tra

### Kiểm tra service status:

```bash
sudo systemctl status bruteforce-detector
```

### Xem logs:

```bash
# Xem logs của service
sudo journalctl -u bruteforce-detector -f

# Xem output detection (chỉ các log phát hiện brute-force)
sudo tail -f /var/ossec/logs/brute.log

# Xem debug logs
sudo tail -f /var/ossec/logs/detector_debug.log
```

## Test với Log Mẫu

### Tạo file test log:

```bash
sudo tee -a /opt/ai-bruteforce/brute.log << 'EOF'
{"timestamp":"2025-11-22T12:46:53.078+0700","agent":{"id":"001","name":"web-server","ip":"192.168.15.10"},"manager":{"name":"Wazuh"},"id":"1763790413.16245","full_log":"{\"timestamp\":\"2025-11-22T12:46:51.464+0700\",\"username\":\"admin\",\"status_code\":320,\"ip\":\"192.168.15.12\",\"password\":\"pofanse\"}","decoder":{"name":"json"},"data":{"timestamp":"2025-11-22T12:46:51.464+0700","username":"admin","status_code":"320","ip":"192.168.15.12","password":"pofanse"},"location":"/var/log/dvwa_auth.log"}
EOF
```

### Kiểm tra output:

```bash
# Chờ vài giây để service xử lý, sau đó check
sudo tail -f /var/ossec/logs/brute.log
```

## Quản lý Service

```bash
# Start service
sudo systemctl start bruteforce-detector

# Stop service
sudo systemctl stop bruteforce-detector

# Restart service
sudo systemctl restart bruteforce-detector

# Check status
sudo systemctl status bruteforce-detector

# Enable auto-start on boot
sudo systemctl enable bruteforce-detector

# Disable auto-start on boot
sudo systemctl disable bruteforce-detector

# View logs
sudo journalctl -u bruteforce-detector -f

# View last 100 lines
sudo journalctl -u bruteforce-detector -n 100
```

## Cấu hình Detection

File: `/root/ai-brute/rebornAI/rebornAI/realtime_bruteforce_detector.py`

### Detection Threshold

```python
DETECTION_THRESHOLD = -0.05  # Hoặc None để dùng model default
```

### Rule-based Detection

```python
RULE_1_ENABLED = False   # Username enumeration
RULE_2_ENABLED = False   # Password spraying
RULE_3_ENABLED = False   # High request rate
RULE_4_ENABLED = False   # False positive filter
```

Để bật rule-based detection, set `True` cho các rule tương ứng.

## Troubleshooting

### Service không start

1. **Kiểm tra logs:**
```bash
sudo journalctl -u bruteforce-detector -n 50
```

2. **Kiểm tra model file:**
```bash
ls -lh /root/ai-brute/rebornAI/rebornAI/models/optimized_bruteforce_detector.pkl
```

3. **Kiểm tra Python path:**
```bash
which python3
python3 --version
```

4. **Kiểm tra dependencies:**
```bash
python3 -c "import pandas, sklearn, joblib, numpy; print('OK')"
```

### Không phát hiện brute-force

1. **Kiểm tra threshold:**
   - Mở file `realtime_bruteforce_detector.py`
   - Điều chỉnh `DETECTION_THRESHOLD` nếu cần

2. **Kiểm tra input log:**
```bash
sudo tail -f /opt/ai-bruteforce/brute.log
```

3. **Kiểm tra service logs:**
```bash
sudo journalctl -u bruteforce-detector -f
```

### Output file không được tạo

1. **Kiểm tra quyền:**
```bash
sudo ls -la /var/ossec/logs/
```

2. **Tạo file nếu chưa có:**
```bash
sudo touch /var/ossec/logs/brute.log
sudo chmod 644 /var/ossec/logs/brute.log
```

3. **Kiểm tra service có quyền ghi:**
```bash
sudo journalctl -u bruteforce-detector | grep -i "permission\|denied"
```

## Lưu ý Quan Trọng

1. **Path Script**: `/root/ai-brute/rebornAI/rebornAI/realtime_bruteforce_detector.py`
2. **Real-time Processing**: Service sẽ đọc log mới ngay khi có (không đợi window interval)
3. **Chỉ ghi brute-force**: Chỉ các log phát hiện brute-force mới được ghi vào output file
4. **Auto-restart**: Service tự động restart nếu crash (RestartSec=10)
5. **Memory management**: Tự động cleanup processed alerts để tránh memory leak
6. **Performance**: Sử dụng tail-like reading để tối ưu performance

## Kiểm tra Logic Code

### Logic Real-time Detection:

1. **Đọc log mới**: Sử dụng `read_new_logs_from_position()` để đọc log mới từ vị trí cuối cùng
2. **Detect ngay**: Mỗi log mới được detect ngay lập tức
3. **Tránh duplicate**: Sử dụng `processed_alert_logs` set để tránh ghi trùng
4. **Chỉ ghi brute-force**: Chỉ các log phát hiện brute-force được ghi vào output file

### Logic Detection Result Unpack:

- **Format mới**: 9 elements (is_bruteforce, normalized_score, patterns, confidence, threat_level, risk_score, raw_anomaly_score, decision_threshold, features)
- **Fallback**: Hỗ trợ format cũ (6 elements) để backward compatibility
- **Error handling**: Log lỗi và skip nếu format không hợp lệ

## Hỗ trợ

Nếu gặp vấn đề, kiểm tra:
1. Service logs: `journalctl -u bruteforce-detector -f`
2. Debug logs: `/var/ossec/logs/detector_debug.log`
3. Model file: `/root/ai-brute/rebornAI/rebornAI/models/optimized_bruteforce_detector.pkl`

